...
(async () => {
  console.log("Looking for item 256...");

  // First, find item 256
  const { data: findData, error: findError } = await supabase
    .from('products')
    .select('*')
    .eq('inventory_number', 256)
    .single();
  ...
  console.log('Found item 256:', findData.name);
  console.log('\n');

  // Mark it as sold with status='active' and sold_at timestamp
  console.log("Marking item 256 as sold...");
  ...
  if (error) {
    ...
    console.log('✅ Successfully marked item 256 as sold!');
